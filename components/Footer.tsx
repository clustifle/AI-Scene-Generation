...
const Footer: React.FC<FooterProps> = ({ isOnDressingScreen = false }) => {
  ...
  return (
    <footer className={`fixed bottom-0 left-0 right-0 bg-surface/80 backdrop-blur-md border-t border-outline/60 p-3 z-50 ${isOnDressingScreen ? 'hidden sm:block' : ''}`}>
      <div className="mx-auto flex flex-col sm:flex-row items-center justify-between text-xs text-onSurfaceVariant max-w-7xl px-4">
        <p>
          Created by{' '}
          <a 
            ...
            className="font-semibold text-onSurface hover:underline"
          >
            @ammaar
          </a>
          {' | modified and enhanced by Clustifle'}
        </p>
